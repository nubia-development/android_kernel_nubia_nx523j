...
struct smbchg_chip {
	...
	struct delayed_work	typec_en_work;
	int                 cc_gpio_en;
	#endif
};
...
#ifdef CONFIG_ZTEMT_CHARGER_CNTL_TYPEC
static int smbchg_typec_ic_gpio_init(struct smbchg_chip *chip)
{
    int rc;
	
	chip->cc_gpio_en = of_get_named_gpio_flags(chip->dev->of_node, "typec,enable-gpio", 0, NULL);
	rc = gpio_request(chip->cc_gpio_en, "chg_usb_cc_en");
	if (rc < 0) {
		pr_err("gpio_request_array failed");
		return rc;
	}

	rc = gpio_direction_output(chip->cc_gpio_en, 0);
	if(rc < 0){
		pr_err("fail to output cc_gpio_en, rc=%d \n",rc);
		return rc;
	}
...
static int smbchg_typec_ic_enable(struct smbchg_chip *chip, bool enable)
{
	ztechg_info("set typec ic cc_gpio_en[%d] enable=%d\n",chip->cc_gpio_en,enable);
	
	gpio_set_value(chip->cc_gpio_en, enable? 1 : 0);
    return 0;
}

static bool is_otg_present(struct smbchg_chip *chip);
static irqreturn_t usbid_change_handler(int irq, void *_chip);
static void typec_en_worker(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work, struct smbchg_chip, typec_en_work.work);
	bool otg_present = is_otg_present(chip);

    ztechg_info("worker enter: usb_present=%d otg_present=%d\n",chip->usb_present,otg_present);
...
static void handle_usb_removal(struct smbchg_chip *chip)
{
	...
	#ifdef CONFIG_ZTEMT_CHARGER_CNTL_TYPEC
	smbchg_typec_ic_enable(chip, true);
	#endif
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
    ...
    #ifdef CONFIG_ZTEMT_CHARGER_CNTL_TYPEC
	smbchg_typec_ic_enable(chip, false);
    #endif
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		val->intval = chip->batt_full_design_mah * 1000;
		break;
	#endif
	default:
		return -EINVAL;
...
static int determine_initial_status(struct smbchg_chip *chip)
{
	...
	if (chip->usb_present) {
		...
		handle_usb_removal(chip);
		#ifdef CONFIG_ZTEMT_CHARGER_CNTL_TYPEC
		schedule_delayed_work(&chip->typec_en_work, msecs_to_jiffies(300));
		#endif
	}
...
static int smb_parse_dt(struct smbchg_chip *chip)
{
	...
	#ifdef CONFIG_ZTEMT_CHARGER_CNTL_TYPEC
	smbchg_typec_ic_gpio_init(chip);
	#endif
...
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	#ifdef CONFIG_ZTEMT_CHARGER_CNTL_TYPEC
	INIT_DELAYED_WORK(&chip->typec_en_work, typec_en_worker);
	#endif
	#ifdef CONFIG_ZTEMT_COMMON_CHARGER
