	...
	struct msm_vfe_axi_stream *stream_info)
{
	...
	for (i = 0; i < stream_info->num_planes; i++) {
		if (stream_info->state == START_PENDING ||
			stream_info->state == RESUME_PENDING ||
			stream_info->state == STARTING) {//ZTEMT: li.bin223 add for video split by qualcomm
			vfe_dev->hw_info->vfe_ops.axi_ops.
...
void msm_isp_axi_stream_update(struct vfe_device *vfe_dev,
	enum msm_vfe_input_src frame_src)
{
	int i, vfe_id;//ZTEMT: li.bin223 add for video split by qualcomm
	unsigned long flags;
	struct msm_vfe_axi_shared_data *axi_data = &vfe_dev->axi_data;
	//ZTEMT: li.bin223 add for video split by qualcomm --start
...
static int msm_isp_update_stream_bandwidth(struct vfe_device *vfe_dev)
{
	...
	//uint64_t stats_ab = 0, stats_ib = 0; //ZTEMT: li.bin223 delete for enlarge bandwidth
